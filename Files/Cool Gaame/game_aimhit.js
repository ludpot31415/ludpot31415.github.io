...
function startGame() {
    ...
    greenfield = new component(200, 400, "rgba(0,250,0,0.4)", 400, 0);
    leftyellowfield = new component(200, 400, "rgba(250,250,0,0.4)", 200, 0);
    rightyellowfield = new component(200, 400, "rgba(250,250,0,0.4)", 600, 0);
    leftredfield = new component(200, 400, "rgba(250,0,0,0.4)", 0, 0);
    rightredfield = new component(200, 400, "rgba(250,0,0,0.4)", 800, 0);
    myGameArea.start();
    document.getElementById('startButtons').remove();
    document.getElementById('regularScript').remove();
}
...
function givePoints() {
    /* Check for points in GREEN */
    if (playerobj.x > greenfield.x) {
            if (playerobj.x < greenfield.x + greenfield.width) {
                points += 5;
                popUp(0)
            }
        }

    /* Check for points in LEFTYELLOW */
    if (playerobj.x > leftyellowfield.x) {
        if (playerobj.x < leftyellowfield.x + leftyellowfield.width) {
            points += 2
            popUp(1)
        }
    }

    /* Check for points in RIGHTYELLOW */
    if (playerobj.x > rightyellowfield.x) {
        if (playerobj.x < rightyellowfield.x + rightyellowfield.width) {
            points += 2
            popUp(1)
        }
    }
    
    /* Check for points in LEFTRED */
    if (playerobj.x > leftredfield.x) {
        if (playerobj.x < leftredfield.x + leftredfield.width) {
            lives -= 1;
            popUp(2)
        }
    }

    /* Check for points in RIGHTRED */
    if (playerobj.x > rightredfield.x) {
        if (playerobj.x < rightredfield.x + rightredfield.width) {
            lives -= 1;
            popUp(2)
        }
    }
...
function levelUp() {
    ...
    greenfield.x += (wallchange / 2);
    greenfield.width -= wallchange;
    
    leftyellowfield.x += (wallchange + wallchange / 2);
    leftyellowfield.width -= (wallchange);

    rightyellowfield.x -= (wallchange / 2);
    rightyellowfield.width -= (wallchange);

    leftredfield.width += (wallchange + wallchange / 2);

    rightredfield.x -= (wallchange + wallchange / 2);
    rightredfield.width += (wallchange + wallchange / 2);
...
function updateGameArea() {
    ...
    moveRedGuy();
    greenfield.update();
    leftyellowfield.update();
    rightyellowfield.update();
    leftredfield.update();
    rightredfield.update();
    playerobj.update();
    if (lives == 0) {
        popUp(4)
        resetGame()
