...
Synth.loadSoundProfile({
	name: 'piano',
	attack: function() { return 0.15; },
	dampen: function(sampleRate, frequency, volume) {
		return Math.pow(0.5*Math.log((frequency*volume)/sampleRate),1.4);
	},
	wave: function(i, sampleRate, frequency, volume) {
		...
		return this.modulate[1](
			...
			sampleRate,
			frequency,
			Math.pow(base(i, sampleRate, frequency, 0), 0) +
				(0 * base(i, sampleRate, frequency, 0.0)) +
				(0 * base(i, sampleRate, frequency, 0))
		);
	}
},
{
...
{
	name: 'edm',
	attack:	function() { return 0.000; },
	dampen: function() { return 0; },
	wave: function(i, sampleRate, frequency) {
		var base = this.modulate[0];
		...
		return mod[0](
			...
			mod[9](
				...
				mod[2](
					...
					sampleRate,
					frequency,
					Math.pow(base(i, sampleRate, frequency, 90
                                 ), 90) +
						Math.pow(base(i, sampleRate, frequency, 0), 0) +
						Math.pow(base(i, sampleRate, frequency, 0), 0)
				)
